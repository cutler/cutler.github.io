title: 优化篇　第一章 基础优化
date: 2016-3-8 10:19:40
categories: Android开发 - 白银
---
　　`“Android开发 - 白银”`分类下的文章介绍知识是中高级`Android`程序员所必备的知识。
...
# 第一节 界面优化 #
　　本节将介绍一些常见的界面优化技巧，通过这些技巧可以避免和查找出各类问题，提高程序运行速度。
## 基础知识 ##

<br>　　**刷新率和帧率**

	-  屏幕刷新率：代表了屏幕一秒内刷新的次数，60Hz就是屏幕一秒内刷新60次。
	-  帧率：代表了GPU在一秒内绘制的帧数，例如60fps就是指GPU一秒钟能生成60张画面。

　　垂直同步：

	-  假设帧率是刷新率的1/2，那么意味着屏幕刷新2次，GPU才能绘制完一幅画面。
	-  相反，如果帧率是刷新率的2倍，那么意味着屏幕刷新1次，GPU能绘制完两幅画面。
	-  高于刷新率的帧数都是无效帧数，对画面效果没有任何提升，反而可能导致画面异常。所以许多游戏都有垂直同步选项，打开垂直同步就是强制游戏帧数最大不超过刷新率。

　　在没有垂直同步的情况下，帧数可以无限高，但刷新率会受到显示器的硬件限制。
...
　　在实际开发中，有如下几种常见情况会导致掉帧。
	...
	UI线程任务繁重。
	-  UI线程是我们的主线程，对于耗时的操作，比如IO操作、网络操作、SQL操作、列表刷新等，应该用后台进程去实现，不能占用主线程。由于界面的绘制也是在UI线程中完成的，因此若UI的任务过多，则就会导致UI延迟绘制、掉帧。
...
<br>　　**垃圾回收**
	...
	-  垃圾回收器用来在应用运行期间自动释放那些不再引用的内存，常称为GC。
	-  GC执行的时候，所有线程的任何操作都会被暂停，等待GC操作完成后，其他操作才能够继续运行。
	-  通常来说，单个的GC并不会占用太多时间，但是大量不停的GC操作就无法能忍受了。
	-  前面提到，平滑的完成一帧意味着所有渲染代码都必须在16ms内完成，但频繁的GC会严重限制一帧时间内的剩余时间（因为GC需要花费处理器时间）。
	-  如果GC所做的工作超过了那些必须的工作，那么留给应用平滑的帧率的时间就越少；越接近16ms，在垃圾回收事件触发的时候，就越容易导致卡顿（即掉帧）。
...
　　Android系统里面有一个`Generational Heap Memory`的模型。
...
　　在Android开发时，我们使用的大部分都是Java的API（比如使用率非常高`HashMap`）：
	...
	-  所以针对Android这种移动平台，也推出了更符合自己的API，比如SparseArray、ArrayMap用来代替HashMap在有些情况下能带来更好的性能提升。
...
　　这个问题在Java开发的时候也许不算问题，但是在`Android`开发时就不能忍了，所以就有了`ArrayMap`等类。
...
<br>　　首先，这是一个悲伤的故事：
	...
	-  我对此有不同的看法：如果我是面试官的话，会更看重的是面试者解决问题的方法和能力，因为“ListView的优化”这个问题，对代码功力要求的不高，反而对知识储备有要求；甚至于学会一种优化的方法可能只需要几分钟的时间而已。
	-  因此，对于此题，只要面试者能答出常见方式，以及他的解决思路我就会让过。
...
<br>　　最常见的情况，就是为`Button`设置点击监听器了，先来看一个错误的范例：
``` java
...
```
	语句解释：
	-  这样一来，整个ListView中所有的Button就共用一个点击事件监听器了。
...
# 结尾 #
<br>　　相信大家也都知道，性能优化是无止境的，除了上面介绍的通用的优化方法外，我们还可以根据自身App的业务逻辑，来进行进一步的优化。

　　性能优化相关的知识不是区区一篇博客就能概括的了得，所以本文会不定时更新，限于笔者能力有限，暂时就先写这么多吧。
