title: 入门篇　第七章 应用程序资源
date: 2015-2-27 12:56:41
categories: android
---
　　本章的内容主要来自网友翻译的官方教程，笔者仅是稍加整理。

# 第一节 概述 #
　　你应该将你的资源（如图像和字符串）从应用程序代码中分离出来，即外部化资源。这样您就可以提供替代资源（如依据不同的语言或屏幕尺寸）。

<center>
![两个不同屏幕尺寸的设备，使用显示同一个布局时的效果](/img/android/android_4_1.png)
</center>

　　上图中的布局文件是针对小屏幕设备来设计的，因此当它显示在一个大屏幕设备（右）中时就很难看了。
　　此时我们可以再提供一套专为大屏设备设计的布局文件，系统在程序运行时会自动依据设备的尺寸做出选择。如下图所示：
...
# 第二节 提供资源 #
　　你应该尽量将你应用程序中的资源外部化，比如`图片`、`字符串`等，这样有利于你独立处理这些资源。

## 分组资源类型 ##
　　把每一种类型的资源分别放在你的项目中`res/`中特定的子路径下。
<br>　　范例1：项目`res/`下的资源路径子目录。
``` android
目录名			资源类型
animator	存放XML格式的属性动画(Property Animations)。
anim		存放XML格式的View动画(View Animation)。
color		存放XML格式的颜色状态列表。
drawable	存放位图(.png，.9.png，.jpg，.gif)或XML文件。
layout		存放XML格式的布局文件。
menu		存放XML格式的菜单，如选项菜单(Options Menu)，快捷菜单(Context Menu)和子菜单。
raw		存放任意的原始格式文件。
values		存放XML格式的基本数值，如字符串、样式和颜色等。XML文件的根节点为<recources>，我们可以在根节点内部定义各种元素。
xml		任意的XML文件，能在运行时被Resources.getXML()调用。各种XML的配置文件必需存放在此。
```

<br>　　所有保存在上表中提到的子路径下的资源都是你的`“默认”`资源。
...
```
　　如果把这个文件保存为`icon.xml`（在可选资源路径下，如`res/drawable-en-rCA/`）,它将被编译成一个资源，你可以通过`R.drawable.icon`来引用，但它实际上是`R.drawable.icon_ca`的化名。
...
## 提供默认资源 ##
　　如果你的应用程序支持多个语言，那么你应该创建一个不含任何语言和区域修饰语的`values/`路径，并在里面写上默认的语言。
　　因为如果你把所有的字符串都保存在带有语言和地区修饰语的路径下，那么一旦设备设置了一个你不支持的语言和区域，你的应用程序将崩溃。

## Android怎样寻找最佳匹配资源 ##
　　假设下列`drawable`路径每一个都包含了不同版本的同一个图片：
...
　　不要手动修改`R.java`文件——它是在项目被编译时由`aapt`生成的。
...
```
　　在本范例中，Android将使用`android:textColorSecondary`样式属性的值作为`android:textColor`的值。
　　编译工具知道，在这个环境下需要一个属性资源，因此并不需要明确指定其类型（即`?android:attr/textColorSecondary`），此处可省写`attr`。

# 第四节 资源类型 #
　　本节的每小节都描述了一类应用资源的用法，格式和语法。你可以把这些资源放在你的资源目录下（即`res/`)。

## Animation ##
　　动画相关的知识，请参阅[《媒体篇　第三章 动画》](http://cutler.github.io/android-D03/)。
...
<br>　　比如手机显示剩余电量的功能，就可以由等级列表来完成（当剩余`80%`电量时显示某张图，其他以此类推）。
