...
　　笔者认为，严格意义的来说Bundle并不算是真正的跨进程通信，因为本地组件只能发送请求，没法方便的获取远程组件的返回值，即通信是单向的：

	-  对于远程的Activity来说，虽然可以通过setResult方法返回值，但是必须得同时finish它自己。
	-  对于远程的服务和广播来说，只能通过发送广播的方式通知本地组件。
...
<br>　　AIDL的大致工作流程（为了方便理解，笔者简写了一些东西，后面会具体介绍）：
